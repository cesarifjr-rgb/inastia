...
- `src/client.ts` gère l'interface commune ; `src/entries/` initialise les fonctionnalités propres aux pages, sélectionnées dans `src/templates.ts`. `src/contact.ts` gère le formulaire. Préserver le chargement ciblé des scripts et des styles, ainsi que les budgets de `tests/page-resources.spec.ts` ; voir `docs/frontend-resources.md`.
...
- La demande du 25 septembre 2026 confirme que `conciergerie-privee-corse` est disponible dès maintenant, sans échéance de lancement ni mention de saison future. Cette offre est réservée aux voyageurs : ne pas réintroduire de parcours ni de CTA propriétaires dans la conciergerie privée. Les fichiers `private-concierge` portent ses contenus et sa présentation FR/EN. Présenter les services comme disponibles dès maintenant, sur devis et selon disponibilités, sans réservation garantie ni tarifs non validés. Les professionnels et créneaux sont confirmés pour chaque demande. Les CTA ouvrent un email prérempli pour le séjour du voyageur ; le formulaire de gestion conserve son périmètre.
- La page présente un catalogue illustré de services à la carte (chef, garde d’enfants, courses, bien-être, bateau, voiture, linge, événements, etc.), avec un devis prérempli par service, puis l’accompagnement des voyageurs et les modalités à la carte, pour un séjour et pour un événement. L’intendance conserve sa page ; les anciens packs internes d’heures ne sont pas l’offre commerciale de référence.
