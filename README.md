...
`contactPath(locale, intent)` conserve le motif choisi dans l’URL de contact : `audit` ou `gestion`. Les principaux appels à l’action utilisent « Demander une proposition de gestion » ; l’audit reste une première étape possible. L’interface propose également un échange général. Les anciennes valeurs `annonce` et `rotation` reviennent à ce choix neutre côté client ; l’API continue à accepter ces anciens payloads pour compatibilité, sans proposer d’offres partielles. Le formulaire adapte le titre, les aides, le bouton et le message de réussite au motif choisi, puis le transmet avec la demande. Pour l’audit gratuit, Inastia rappelle sous 24 h selon la convenance du propriétaire : ce délai concerne le premier échange, pas la réalisation de l’analyse. Pour les autres demandes, le visiteur choisit une réponse par email ou téléphone. Le téléphone est obligatoire pour l’audit, une réponse demandée par téléphone ou les appels commerciaux, côté formulaire et API ; sinon il est masqué et exclu du payload. La préférence de réponse ne vaut pas consentement commercial. Le nom de famille et le projet sont facultatifs ; l’aide invite à préciser les disponibilités pour un rappel d’audit.

La [matrice des responsabilités](docs/responsibility-matrix.md) rapproche le périmètre annoncé et les frais de la gestion complète. Le [suivi des demandes qualifiées](docs/copywriting-measurement.md) fournit une grille privée à remplir et une méthode de bilan manuel, sans connexion au Hub ni mesure d’audience automatique.

L’offre détaille sept domaines sur l’accueil et la page gestion FR/EN : annonces, réservations et voyageurs, ménage/linge/consommables, tarification et calendrier, arrivées/départs/clés, assistance et maintenance, cautions et suivi des dommages. L’assistance voyageurs 24 h/24, 7 j/7 est confirmée par Inastia. Les trois illustrations restent une vue d’ensemble de cette gestion de A à Z ; les honoraires, prestations liées aux séjours et dépenses du logement sont identifiés séparément. La disponibilité de l’assistance n’est pas un délai de réparation sur place ni une garantie d’indemnisation.
