...
`src/motion.ts` gère les entrées natives avec Web Animations et IntersectionObserver, la pause/reprise et la visibilité. Les entrées déplacent le contenu de 12 px pendant 220 ms sans le masquer ; le focus annule le mouvement. Le CSS anime le porte-clés, la clé et les vagues lorsque l’illustration est active. La préférence `prefers-reduced-motion` initialise une pause ; l’utilisateur peut explicitement reprendre. Respecter cet état, la suspension hors écran/onglet masqué et la reprise BFCache. Une capture fixe ne démontre pas la qualité temporelle du mouvement.
