...
export async function syncEnquiry(input, context) {
    ...
    const phone = phoneIdentity(input.phone);
    if (!email && !phone) throw new Error('attio_missing_contact_channel');
    const matches = await api('/people/records/query', 'POST', { filter: email ? { email_addresses: email } : { phone_numbers: phone }, limit: 2 });
    if (matches.length > 1) throw new Error('attio_ambiguous_email');
    // Do not replace an existing contact's additional email addresses.
    const person = matches[0] || (email
        ? await api('/people/records?matching_attribute=email_addresses', 'PUT', { data: { values: { email_addresses: [email] } } })
        : await api('/people/records', 'POST', { data: { values: { phone_numbers: [phone] } } }));
    ...
    const values = {
        ...
        site_derniere_demande: [
            ...
            `Version du site déclarée : ${input.siteVersion || 'non fournie'}`,
            ...
            `Offres email : ${context.marketingEmail ? 'acceptées' : input.consentVersion ? 'refusées' : 'aucun choix fourni'}`,
            `Appels commerciaux : ${context.marketingPhone ? 'acceptés, un an maximum' : input.consentVersion ? 'refusés' : 'aucun choix fourni'}`,
