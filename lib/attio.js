// Called only for a verified, registered enquiry (or accepted legacy email).
import { createHash } from 'node:crypto';
import { journeyAttribution } from './journey.js';
import { acquisitionAttribution } from './acquisition.js';
...
export async function syncEnquiry(input, context, { timeoutMs = 45000 } = {}) {
    ...
    const journey = journeyAttribution(context.journey, context.receivedAt);
    const acquisition = acquisitionAttribution(context.acquisition, context.receivedAt);
    const source = acquisition ? 'Google Business Profile — inastia.fr'
        : context.ads ? 'Google Ads — inastia.fr' : 'inastia.fr — canal non attribué';
    const values = {
        site_derniere_demande_id: context.requestId,
        site_derniere_demande_le: new Date(context.receivedAt).toISOString(),
        site_source: acquisition || context.ads ? source : 'inastia.fr',
        site_derniere_demande: [
            ...
            ...(acquisition ? ['Origine de la visite (mesure consentie) : Google Business Profile — lien balisé',
                `Arrivée enregistrée par le navigateur : ${new Date(acquisition.at).toISOString()}`] : []),
            ...(journey ? [`Origine du CTA (mesure consentie) : ${journey.page} / ${journey.placement} / ${journey.locale}`] : []),
            ...(input.intent === 'audit' ? ['Audit gratuit exploratoire : aucune demande de gestion complète ni décision de déléguer ne peut en être déduite.'] : []),
    ...
    const initialDeal = {
        name: input.intent === 'audit' ? `Audit gratuit — ${name}` : name, stage: 'Nouveau lead', owner: 'contact@inastia.fr',
        ...
        prochaine_action: input.intent === 'audit' ? 'Préparer le rappel d’audit gratuit selon les disponibilités du demandeur ; aucune décision de gestion complète à présumer.' : `Vérifier le projet et répondre par ${context.contactPreference === 'phone' ? 'téléphone' : 'email'}.`,
    ...
    if (journey) Object.assign(initialDeal, { site_page_origine: journey.page,
        site_emplacement_cta: journey.placement, site_langue_origine: journey.locale });
