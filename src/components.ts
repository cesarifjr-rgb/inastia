...
export function footer(locale: Locale): string {
  ...
  </div><div class="container footer-bottom"><span>© ${new Date().getFullYear()} Inastia</span><nav aria-label="${t(locale, "Informations légales", "Legal information in French")}"><a href="/mentions-legales" lang="fr">${t(locale, "Mentions légales", "Legal notice (FR)")}</a><a href="/privacy" lang="fr">${t(locale, "Confidentialité", "Privacy (FR)")}</a><a href="/cgv" lang="fr">${t(locale, "CGV", "Terms (FR)")}</a><button id="ads-consent-settings" type="button" hidden>${t(locale, "Cookies publicitaires", "Advertising cookies")}</button></nav><a href="#main">${t(locale, "Retour en haut", "Back to top")} ↑</a></div></footer><section id="ads-consent" class="ads-consent" role="region" aria-labelledby="ads-consent-title" tabindex="-1" hidden><h2 id="ads-consent-title">${t(locale, "Mesurer nos annonces, avec votre accord", "Measure our ads, with your consent")}</h2><p>${t(locale, "Acceptez-vous les cookies de Google Ads pour relier vos visites, vos demandes envoyées et leurs suites (qualification, mandat signé) à nos annonces ? Google reçoit des données de navigation et des identifiants publicitaires, mais pas les coordonnées ni le message de votre formulaire. Votre choix est conservé 6 mois et peut être modifié via « Cookies publicitaires » en bas de page.", "Do you accept Google Ads cookies to link your visits, submitted enquiries and their outcomes (qualification, signed mandate) to our ads? Google receives browsing data and advertising identifiers, but no contact details or message from your form. Your choice is saved for 6 months and can be changed through ‘Advertising cookies’ in the footer.")} <a href="/privacy#publicite" lang="fr">${t(locale, "En savoir plus", "Learn more (French)")}</a>.</p><div class="ads-consent-actions"><button type="button" data-ads-choice="reject">${t(locale, "Refuser", "Reject")}</button><button type="button" data-ads-choice="accept">${t(locale, "Accepter", "Accept")}</button></div></section>`;
}
