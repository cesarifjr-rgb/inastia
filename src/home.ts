...
export function home(locale: Locale): string {
  ...
  <section class="home-hero">
    ...
    <div class="hero-main container">
      ...
      <div class="hero-actions"><a class="button" href="${contactPath(locale, "gestion")}">${t(locale, "Parlons de votre logement", "Let’s talk about your property")}${arrow}</a><a class="text-link" href="#tarifs">${t(locale, "Comprendre les honoraires", "Understand our fees")}</a></div>
  ...
  <nav class="container home-rental-paths" aria-label="${t(locale, "Votre projet de location", "Your rental plans")}">
    <a href="${path(locale, firstRentalSlug)}"><span><strong>${t(locale, "Première mise en location", "Your first holiday rental")}</strong><span>${t(locale, "Préparer votre première saison, étape par étape", "Prepare for your first season, step by step")}</span></span>${arrow}</a>
    <a href="${path(locale, "gestion-airbnb-corse-du-sud")}#reprise-gestion"><span><strong>${t(locale, "Logement déjà en location", "Already renting out your property")}</strong><span>${t(locale, "Comprendre comment se prépare la reprise de gestion", "See how we prepare the management handover")}</span></span>${arrow}</a>
  </nav>
