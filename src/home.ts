...
export function home(locale: Locale): string {
  ...
  <section class="home-hero">
    ...
    <div class="hero-main container">
      ...
      <p class="hero-description">${t(locale, "Annonce, réservations, voyageurs et suivi sur place : notre équipe familiale coordonne la gestion complète de votre location, de Ghisonaccia à Porto-Vecchio. Vous nous confiez le quotidien, vous gardez vos séjours personnels.", "Listing, bookings, guests and local care: our family team coordinates full management of your rental, from Ghisonaccia to Porto-Vecchio. We take care of the day-to-day work; you keep your own stays.")}</p>
  ...
  <section class="section services-section" id="services"><span id="formules" class="anchor-target" aria-hidden="true"></span><span id="comparaison" class="anchor-target" aria-hidden="true"></span><div class="container">
    <div class="section-heading" data-reveal><div><p class="eyebrow">${t(locale, "01 — LA GESTION COMPLÈTE", "01 — FULL MANAGEMENT")}</p><h2>${t(locale, "Une seule offre.<br><em>Tout un quotidien pris en main.</em>", "One service.<br><em>Your day-to-day management, handled.</em>")}</h2></div><p>${t(locale, "L’annonce, les voyageurs et le suivi de la maison : trois volets coordonnés par la même équipe. Nous proposons exclusivement la gestion complète de votre location, sans prestations séparées.", "Your listing, guests and local care: three parts coordinated by the same team. We provide full rental management exclusively; these services are not offered separately.")}</p></div>
  ...
  <section class="presence-section"><div class="container">
    <div class="presence-grid"><div class="presence-heading" data-reveal><p class="eyebrow">${t(locale, "02 — UNE ÉQUIPE FAMILIALE", "02 — A FAMILY TEAM")}</p><h2>${t(locale, "Votre maison.<br><em>Notre présence.</em>", "Your home.<br><em>Our local care.</em>")}</h2></div><div class="presence-copy" data-reveal><p class="presence-lead">${t(locale, "Déléguer votre location commence par savoir à qui vous confiez vos clés.", "Handing over your rental starts with knowing who will hold your keys.")}</p><p>${t(locale, "Propriétaires nous-mêmes, nous savons que votre maison compte au-delà des réservations. Vous échangez directement avec notre équipe familiale. Nous coordonnons le quotidien et vous signalons les points qui demandent votre décision, pour que vous restiez informé même à distance.", "As owners ourselves, we know your home matters beyond its bookings. You speak directly with our family team. We coordinate the day-to-day work and flag decisions for you, keeping you informed even when you are away.")}</p><a class="text-link" href="${path(locale, "about")}">${t(locale, "Découvrir notre façon de travailler", "Discover how we work")}${arrow}</a></div></div>
