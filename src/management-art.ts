...
export function managementArt(locale: Locale): string {
  ...
  return `<section class="section management-art-section" id="gestion-au-quotidien"><div class="container"><div class="management-art" data-management-art>
    <div class="management-art-heading"><p class="eyebrow">${t(locale, "LA GESTION COMPLÈTE", "FULL MANAGEMENT")}</p><h2>${t(locale, "Votre location,<br>prise en charge de A à Z.", "Every part of your rental,<br>taken care of.")}</h2></div>
    ...
    <div class="management-art-copy">
      <div data-management-copy="listing"><span class="management-art-index" aria-hidden="true">01</span><h3>${t(locale, "Avant la réservation", "Before a booking")}</h3><p>${t(locale, "Photos professionnelles, diffusion de l’annonce, ajustement des prix et calendrier : nous préparons la mise en location et traitons les demandes de réservation.", "Professional photography, listing distribution, pricing and availability: we prepare your rental and handle booking enquiries.")}</p></div>
      <div data-management-copy="guests"><span class="management-art-index" aria-hidden="true">02</span><h3>${t(locale, "Avant et pendant le séjour", "Before and during the stay")}</h3><p>${t(locale, "Nous gérons les messages, les consignes, les clés, les arrivées et les départs. Pendant tout le séjour, les voyageurs disposent de notre assistance 24 h/24, 7 j/7.", "We handle messages, instructions, keys, arrivals and departures. Throughout their stay, guests have access to our 24/7 assistance.")}</p></div>
      <div data-management-copy="home"><span class="management-art-index" aria-hidden="true">03</span><h3>${t(locale, "Entre deux séjours", "Between stays")}</h3><p>${t(locale, "Nous organisons le ménage, le linge propre et les réassorts, puis vérifions la maison. En cas de besoin, nous suivons les réparations, les cautions et les dossiers de dommages.", "We arrange cleaning, fresh linen and restocking, then check the home. When needed, we follow up repairs, deposits and damage claims.")}</p></div>
    </div>
  </div></div></section>`;
}
