...
export function managementArt(locale: Locale): string {
  ...
  return `<section class="section management-art-section" id="gestion-au-quotidien"><div class="container"><div class="management-art" data-management-art>
    <div class="management-art-heading"><p class="eyebrow">${t(locale, "Un même accompagnement", "One coordinated service")}</p><h2>${t(locale, "Votre maison.<br>Le soin des détails.", "Your home.<br>Care in every detail.")}</h2></div>
    ...
    <div class="management-art-copy">
      <div data-management-copy="listing"><span class="management-art-index" aria-hidden="true">01</span><h3>${t(locale, "Une annonce suivie", "A listing looked after")}</h3><p>${t(locale, "Présentation, prix et calendrier : le suivi de votre annonce s’inscrit dans le périmètre convenu ensemble.", "Presentation, pricing and calendar: your listing is managed within the scope agreed together.")}</p></div>
      <div data-management-copy="guests"><span class="management-art-index" aria-hidden="true">02</span><h3>${t(locale, "Le lien avec les voyageurs", "Keeping in touch with guests")}</h3><p>${t(locale, "Les échanges et l’organisation des arrivées sont coordonnés selon les prestations définies pour votre logement.", "Guest communication and arrival arrangements are coordinated according to the services agreed for your home.")}</p></div>
      <div data-management-copy="home"><span class="management-art-index" aria-hidden="true">03</span><h3>${t(locale, "Un relais pour la maison", "Local support for your home")}</h3><p>${t(locale, "Ménage, linge et suivi sur place sont organisés selon vos besoins et le cadre convenu. Vous gardez les décisions qui vous reviennent.", "Cleaning, linen and local follow-up are organised around your needs and the agreed scope. You keep the decisions that remain yours.")}</p></div>
    </div>
  </div></div></section>`;
}
