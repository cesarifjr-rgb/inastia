...
import { arrow, contactPath, path, t } from "./lib.ts";

export function pricing(locale: Locale, detailed = true): string {
  const included = [
  ...
  return `<section class="pricing-section" id="tarifs" aria-labelledby="pricing-title"><div class="container">
    <div class="pricing-top">
      ...
      <details class="pricing-calculator" data-pricing-calculator data-locale="${locale}" aria-labelledby="pricing-example-title">
        <summary>${t(locale, "Comprendre le calcul des honoraires", "Understand the fee calculation")}</summary><div class="pricing-calculator-bar"><span aria-hidden="true">● ● ●</span><span>INASTIA · ${t(locale, "VOTRE CALCUL", "YOUR CALCULATION")}</span></div>
        <div class="pricing-calculator-body"><h3 id="pricing-example-title" class="eyebrow">${t(locale, "COMPRENDRE LE CALCUL DE VOS FRAIS DE GESTION", "UNDERSTAND HOW YOUR MANAGEMENT FEES ARE CALCULATED")}</h3>
          ...
          <a class="button button-cream" href="${contactPath(locale, "gestion")}">${t(locale, "Demander ma proposition de gestion", "Request my management proposal")}${arrow}</a>
        </div>
      </details>
    </div>
    <div class="pricing-detail" id="detail-tarif"><div class="pricing-detail-heading"><p class="eyebrow">${t(locale, "LE DÉTAIL DU TARIF", "THE FEE IN DETAIL")}</p><h3>${t(locale, "Tout est géré.<br><em>Chaque coût est expliqué.</em>", "Everything is managed.<br><em>Every cost is explained.</em>")}</h3><p>${t(locale, "La gestion à 100 % signifie que nous prenons en charge toutes les étapes de votre location. La commission rémunère cette gestion. Le ménage, le linge et les consommables sont facturés au locataire ; les réparations et autres dépenses du logement restent séparées, avec votre accord.", "Complete management means we take care of every stage of your rental. The commission pays for that management. Cleaning, linen and supplies are charged to the guest; repairs and other property expenses are separate and require your approval.")}</p></div>
      <div class="pricing-columns"><article class="pricing-card pricing-card-included"><h4><span aria-hidden="true">✓</span>${t(locale, "Honoraires de gestion", "Management fees")}</h4>${detailed ? list(included) : `<p>${t(locale, "La gestion de votre location présentée ci-dessus, de l’annonce au suivi de votre maison.", "The rental management described above, from your listing to the care of your home.")}</p><a class="text-link" href="${path(locale, "gestion-airbnb-corse-du-sud")}#section-1">${t(locale, "Consulter le périmètre complet", "Read the full service scope")}${arrow}</a>`}<p>${t(locale, "Couverts par la commission de 20 % TTC, facturée au propriétaire.", "Covered by the 20% commission including VAT, invoiced to the owner.")}</p></article><article class="pricing-card"><h4>${t(locale, "Prestations liées au séjour", "Services linked to stays")}</h4>${list(stay)}<p>${t(locale, "Le devis identifie le payeur et le montant de chaque prestation. Ces services concernent les locations en gestion complète. L’intendance des résidences secondaires dispose de ses propres forfaits et suppléments.", "The quote identifies who pays and how much each service costs. These services apply to fully managed rentals. Second-home care has its own plans and additional-service prices.")}</p></article><article class="pricing-card pricing-card-agreed"><h4>${t(locale, "Entretien du logement", "Property maintenance")}</h4>${list(maintenance)}<p>${t(locale, "Dépenses à votre charge, distinctes des honoraires de gestion. Les besoins et les coûts vous sont présentés pour décision.", "Costs borne by you, separate from management fees. We present the work needed and its cost so you can decide.")}</p></article><article class="pricing-card"><h4>${t(locale, "Obligations du propriétaire", "Owner’s responsibilities")}</h4>${list(ownerCosts)}<p>${t(locale, "Ces frais et obligations sont à traiter séparément selon votre logement et votre situation.", "These costs and responsibilities must be addressed separately according to your property and circumstances.")}</p></article></div>
    </div>
  </div></section>`;
}
