...
export function contact(locale: Locale): string {
  ...
  return `<div class="container contact-layout"><div class="contact-intro"><nav class="breadcrumb" aria-label="${t(locale, "Fil d’Ariane", "Breadcrumb")}"><a href="${path(locale)}">${t(locale, "Accueil", "Home")}</a><span aria-hidden="true">/</span><span>Contact</span></nav><p class="eyebrow">${t(locale, "VOTRE PROJET COMMENCE ICI", "YOUR PLANS START HERE")}</p><h1 id="contact-title">${t(locale, "Parlons de votre bien.", "Let’s talk about your property.")}</h1><p id="contact-lead" class="lead">${t(locale, "Présentez-nous votre bien et votre situation. Un premier échange par appel ou email permettra de préciser la gestion complète et le cadre adapté à votre logement.", "Tell us about your property and current situation. An initial conversation by phone or email will help define full management and the arrangements suited to your home.")}</p><div class="contact-direct"><a href="tel:+33613812550">+33 6 13 81 25 50</a><a href="mailto:contact@inastia.fr">contact@inastia.fr</a><p>${t(locale, "De Ghisonaccia à Porto-Vecchio.<br> Une conciergerie familiale, ancrée en Corse.", "From Ghisonaccia to Porto-Vecchio.<br> A family-run service, rooted in Corsica.")}</p></div></div>
  ...
  <div class="field field-full"><label for="message">${t(locale, "Votre projet (facultatif)", "Your plans (optional)")}</label><textarea id="message" name="message" rows="4" maxlength="2000" aria-describedby="message-help"></textarea><p id="message-help" class="field-help">${t(locale, "Précisez votre situation actuelle, votre projet de gestion complète et votre rôle dans la décision. Vous pouvez ajouter la capacité d’accueil et le lien de votre annonce.", "Tell us about your current situation, your full management plans and your role in the decision. You can add the guest capacity and a listing link.")}</p></div></div>
