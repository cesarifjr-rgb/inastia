...
export function contact(locale: Locale): string {
  ...
  <div class="field"><label for="propertyArea">${t(locale, "Quartier, lieu-dit ou adresse du bien", "Property area, neighbourhood or address")} *</label><input id="propertyArea" name="propertyArea" required maxlength="100" aria-describedby="property-area-help"><p id="property-area-help" class="field-help">${t(locale, "Pour vérifier le secteur et les accès : la commune seule ne suffit pas.", "To check the area and access: the town alone is not enough.")}</p></div>
