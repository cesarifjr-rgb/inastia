...
export function secondary(locale: Locale, page: PageContent): string {
  ...
  ${page.kind === "service" ? managementArt(locale) : ""}<section class="section"><div class="container page-sections"><nav class="page-aside" aria-label="${t(locale, "Dans cette page", "On this page")}"><p class="eyebrow">${t(locale, "L’essentiel", "At a glance")}</p>${page.sections.map((section, index) => `<a href="#section-${index + 1}">${escape(section.title)}</a>`).join("")}${handover ? `<a href="#reprise-gestion">${t(locale, "Préparer la reprise de gestion", "Preparing the management handover")}</a>` : ""}</nav><div>${page.sections.map((section, index) => `<section class="editorial-section" data-reveal id="section-${index + 1}"><h2>${escape(section.title)}</h2><p>${escape(section.text)}</p>${section.items?.length ? `<ul>${section.items.map((item) => `<li>${escape(item)}</li>`).join("")}</ul>` : ""}</section>`).join("")}<nav class="related-links" aria-label="${t(locale, "À découvrir aussi", "Explore more")}">${page.kind === "location" ? `<a href="${path(locale, "gestion-airbnb-corse-du-sud")}">${t(locale, "Gestion complète", "Full management")}</a>` : zones.map(([name, slug]) => `<a href="${path(locale, slug)}">${name}</a>`).join("")}</nav></div></div></section>${handoverSection}${page.kind === "service" ? pricing(locale) : ""}${faq(locale, page.faq, intent)}${contactCallout(locale, intent)}`;
}
...
export function contact(locale: Locale): string {
  ...
  <details id="contact-project" class="contact-project" open><summary id="contact-project-summary">${t(locale, "Votre projet (facultatif)", "Your plans (optional)")}</summary><p id="contact-project-help" class="form-note">${t(locale, "Ces précisions peuvent attendre notre premier échange.", "You can share these details during our first conversation.")}</p><div class="form-grid">
