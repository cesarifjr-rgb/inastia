...
export function secondary(locale: Locale, page: PageContent): string {
  ...
  const summaryTitle = page.kind === "audit"
    ? t(locale, "Les priorités pour préparer votre projet.", "Priorities to prepare your plans.")
    : page.kind === "about"
...
export function contact(locale: Locale): string {
  ...
  <div class="field"><label for="propertyArea">${t(locale, "Quartier ou lieu-dit du bien", "Property area or neighbourhood")} *</label><input id="propertyArea" name="propertyArea" required maxlength="100" aria-describedby="property-area-help"><p id="property-area-help" class="field-help">${t(locale, "Pour vérifier le secteur et les accès. L’adresse complète n’est pas nécessaire à ce stade.", "To check the area and access. The full street address is not needed at this stage.")}</p></div>
  <div class="field"><label for="decisionRole">${t(locale, "Votre rôle dans le projet *", "Your role in the project *")}</label><select id="decisionRole" name="decisionRole" required><option value="">${t(locale, "Choisir", "Select")}</option><option value="proprietaire">${t(locale, "Propriétaire", "Owner")}</option><option value="coproprietaire">${t(locale, "Copropriétaire", "Co-owner")}</option><option value="acquereur">${t(locale, "Acquéreur potentiel", "Prospective buyer")}</option><option value="mandataire">${t(locale, "Mandataire autorisé", "Authorised representative")}</option><option value="autre">${t(locale, "Autre", "Other")}</option></select></div>
  <div class="field" data-rental-field><label for="rentalSituation">${t(locale, "Situation locative *", "Rental situation *")}</label><select id="rentalSituation" name="rentalSituation" required><option value="">${t(locale, "Choisir", "Select")}</option><option value="reflexion">${t(locale, "Projet encore en réflexion", "Still exploring the project")}</option><option value="premiere">${t(locale, "Première mise en location", "First rental season")}</option><option value="existante">${t(locale, "Location déjà en activité", "Already renting")}</option><option value="changement">${t(locale, "Changement de conciergerie", "Changing management company")}</option></select></div>
  <div class="field"><label for="startTimeline">${t(locale, "Échéance envisagée *", "Expected timing *")}</label><select id="startTimeline" name="startTimeline" required><option value="">${t(locale, "Choisir", "Select")}</option><option value="desquepossible">${t(locale, "Dès que possible", "As soon as possible")}</option><option value="troismois">${t(locale, "Dans les trois mois", "Within three months")}</option><option value="prochainesaison">${t(locale, "Pour la prochaine saison", "For next season")}</option><option value="adefinir">${t(locale, "À définir ensemble", "To discuss together")}</option></select></div>
  ...
  <div class="field field-full" data-audit-fields hidden><label for="callbackAvailability">${t(locale, "Vos disponibilités pour le rappel (facultatif)", "Your availability for a callback (optional)")}</label><input id="callbackAvailability" name="callbackAvailability" maxlength="300" disabled><p class="field-help">${t(locale, "Indiquez les jours ou créneaux qui vous conviennent.", "Tell us which days or times suit you.")}</p></div>
  ...
  <p class="form-privacy">${t(locale, "Vos informations servent à répondre à votre demande. Le choix de recevoir des offres commerciales vous sera proposé séparément après son envoi.", "We use your details to respond to your enquiry. Marketing preferences will be offered separately after it has been sent.")} <a href="/privacy" lang="fr">${t(locale, "Politique de confidentialité", "Privacy policy (French)")}</a>.</p><div class="contact-verification"><div id="turnstile-container"></div><div id="form-status" role="status" aria-live="polite" tabindex="-1"></div></div><button class="button" id="submit-contact" type="submit"><span id="submit-contact-label">${t(locale, "Envoyer ma demande", "Send my enquiry")}</span>${arrow}</button><button id="form-reset" type="button" hidden>${t(locale, "Envoyer une autre demande", "Send another enquiry")}</button><noscript><p>${t(locale, "L’envoi sécurisé nécessite JavaScript. Vous pouvez nous écrire à", "Secure submission needs JavaScript. You can email us at")} <a href="mailto:contact@inastia.fr">contact@inastia.fr</a> ${t(locale, "ou nous appeler.", "or call us.")}</p></noscript></form><form id="contact-preferences" class="contact-preferences" hidden><h2>${t(locale, "Souhaitez-vous aussi recevoir nos offres ?", "Would you also like to hear about our offers?")}</h2><p>${t(locale, "Votre demande est déjà transmise. Ces choix n’ont aucun effet sur son traitement et vous pouvez simplement quitter cette page.", "Your enquiry has already been sent. These choices do not affect how it is handled, and you can simply leave this page.")}</p>  <fieldset class="contact-consents" aria-describedby="commercial-consent-help"><legend>${t(locale, "Choix commerciaux (facultatifs)", "Marketing preferences (optional)")}</legend><label class="contact-consent" for="marketingEmail"><input type="checkbox" id="marketingEmail" name="marketingEmail"><span>${t(locale, "J’accepte de recevoir par email les offres et relances commerciales d’Inastia concernant la gestion complète de locations.", "I agree to receive Inastia’s offers and marketing follow-ups about full holiday rental management by email.")}</span></label><div class="field" id="preference-email-field" hidden><label for="preference-email">Email *</label><input id="preference-email" type="email" maxlength="254" autocomplete="email" disabled></div><label class="contact-consent" for="marketingPhone"><input type="checkbox" id="marketingPhone" name="marketingPhone"><span>${t(locale, "J’accepte qu’Inastia m’appelle pendant un an pour me présenter ses offres de gestion complète de locations.", "I agree to receive calls from Inastia for one year about its full holiday rental management offers.")}</span></label><div class="field" id="preference-phone-field" hidden><label for="preference-phone">${t(locale, "Téléphone *", "Phone *")}</label><input id="preference-phone" type="tel" maxlength="30" autocomplete="tel" disabled></div><p id="commercial-consent-help" class="field-help">${t(locale, "Ces choix sont facultatifs et indépendants de votre demande. Vous pouvez retirer votre accord à tout moment en écrivant à", "These choices are optional and separate from your enquiry. You can withdraw your consent at any time by emailing")} <a href="mailto:contact@inastia.fr">contact@inastia.fr</a>${t(locale, " ; pour les appels commerciaux, vous pouvez aussi nous le dire pendant l’appel. Le refus de ces offres n’empêche pas le traitement de votre demande ni le rappel d’audit demandé.", "; for marketing calls, you can also tell us during the call. Declining these offers does not prevent us from handling your enquiry or making the audit callback you requested.")}</p></fieldset><button class="button" type="submit">${t(locale, "Transmettre mes choix", "Send my preferences")}</button><p id="preferences-status" role="status" tabindex="-1"></p></form><section class="contact-next-steps" aria-labelledby="contact-next-title"><h2 id="contact-next-title">${t(locale, "Après votre demande", "After your enquiry")}</h2><ol><li>${t(locale, "Notre équipe examine votre commune et vos besoins.", "Our team reviews your town and your needs.")}</li><li>${t(locale, "Nous échangeons sur votre besoin : audit gratuit, gestion complète ou intendance.", "We discuss your enquiry: a free review, full management or home care.")}</li><li>${t(locale, "Nous définissons ensemble les prestations et le devis adaptés à votre bien.", "Together, we define the services and quote suited to your property.")}</li></ol></section></div>`;
}
