...
test("Analytics-only enquiry records a lead only after confirmed success and never sends an Ads conversion", async ({ page }) => {
  ...
  expect((await queue(page)).filter(item => item[1] === "generate_lead")).toEqual([["event", "generate_lead", { send_to: ANALYTICS_ID, form_id: "contact-form", service: "gestion", origin_page: "contact", origin_locale: "fr", contact_placement: "direct" }]]);
